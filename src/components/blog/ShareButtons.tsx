...
interface Props {
  url: string;
  title: string;
  /** slug do post pra rastrear de onde veio o share */
  postSlug?: string;
}

export default function ShareButtons({ url, title, postSlug }: Props) {
  ...
  function trackShare(channel: "whatsapp" | "x" | "linkedin" | "copy") {
    track("blog_share", { channel, post: postSlug ?? "unknown" });
  }
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <ShareLink href={waHref} label="WhatsApp" onClick={() => trackShare("whatsapp")}>
      ...
      <ShareLink href={xHref} label="X (Twitter)" onClick={() => trackShare("x")}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
        </svg>
      </ShareLink>

      <ShareLink href={liHref} label="LinkedIn" onClick={() => trackShare("linkedin")}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
